...
int main(int argc, char *argv[])  
{
	...
	if (rank != 0) {
		...
		rc = MPI_Send(vec[8 * (rank - 1)] + (rank - 1) * 8, 2, MPI_M_STRUCT, 0, tag, MPI_COMM_WORLD);
	} else {
		int i = 1;
	
		for (; i < numtasks; ++i) {
			printf("get from proc %d\n", i);
			MPI_Recv(vec[8 * (i - 1)] + (i - 1) * 8, 2, MPI_M_STRUCT, i, tag, MPI_COMM_WORLD, &status);
